defmodule VirtualPowerPlant do
  ...
  defp server_name do
    Application.get_env(:energy_application, :vpp_name, __MODULE__)
  end
  ...
  def add_battery(battery_id) do
    GenServer.cast(server_name(), {:add_battery, battery_id})
  end

  @doc "Collection of IDs for the batteries we control"
  def batteries do
    GenServer.call(server_name(), :batteries)
  end
  ...
  def current_power do
    GenServer.call(server_name(), :total_power)
  end
  ...
  """
  def export(watts) do
    GenServer.cast(server_name(), {:set_power, watts})
  end
