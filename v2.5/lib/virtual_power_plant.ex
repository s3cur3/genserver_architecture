defmodule VirtualPowerPlant do
  ...
  """
  def add_battery(server \\ __MODULE__, id, max_power_watts, current_power_watts \\ 0) do
    battery = make_battery(id, max_power_watts, current_power_watts)
  ...
  def handle_call({:set_power, needed_watts}, _from, battery_collection) do
    {updated_batteries, _unmet_need} =
      Enum.reduce(battery_collection, {[], needed_watts}, fn battery, {updated_batteries, need} ->
        updated_power = limit_power(need, battery.max_power_watts)
        updated_need = need - updated_power
        updated_battery = %{battery | current_power_watts: updated_power}
  ...
  defp make_battery(id, max_power_watts, current_power_watts) do
    %{id: id, max_power_watts: max_power_watts, current_power_watts: current_power_watts}
  end
